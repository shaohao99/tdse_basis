...
// set offdiagonal DME values, before time iteration
    ...
    *vj2i1=conj(*vi1j2);
    for(int i=0; i<nE; i++){
       ...
       *vj5i2=conj(*vi2j5);
       ...
       for(int j=0; j<nE; j++){
         *vi3j4=5.;  // continuum,cotinuum, by IR
         *vj4i3=conj(*vi3j4);
         vi3j4++;
         vj4i3++;
...
// set offdiagonal DME values, during time iteration
  void DME_time_AIS1(PetscScalar EXt, PetscScalar ELt, IS isi1, IS isi2, IS isi3, IS isi4, IS isi5, IS isi6, IS isj2, IS isj3, IS isj4, IS isj5, IS isj6, IS isj7, Mat &H_temp){
    ...
    MatGetSubMatrix(H_temp,isi1,isj2,1,MAT_INITIAL_MATRIX,&mat_i1j2);  // up triangle
    MatGetSubMatrix(H_temp,isi1,isj3,1,MAT_INITIAL_MATRIX,&mat_i1j3);
    MatGetSubMatrix(H_temp,isi1,isj5,1,MAT_INITIAL_MATRIX,&mat_i1j5);
    MatGetSubMatrix(H_temp,isi1,isj7,1,MAT_INITIAL_MATRIX,&mat_i1j7);
    MatGetSubMatrix(H_temp,isi2,isj4,1,MAT_INITIAL_MATRIX,&mat_i2j4);
    MatGetSubMatrix(H_temp,isi2,isj6,1,MAT_INITIAL_MATRIX,&mat_i2j6);
    MatGetSubMatrix(H_temp,isi3,isj4,1,MAT_INITIAL_MATRIX,&mat_i3j4);
    MatGetSubMatrix(H_temp,isi4,isj5,1,MAT_INITIAL_MATRIX,&mat_i4j5);
    MatGetSubMatrix(H_temp,isi5,isj6,1,MAT_INITIAL_MATRIX,&mat_i5j6);
    MatGetSubMatrix(H_temp,isi6,isj5,1,MAT_INITIAL_MATRIX,&mat_i6j7);
    MatGetSubMatrix(H_temp,isj2,isi1,1,MAT_INITIAL_MATRIX,&mat_j2i1);  // low triangle
    MatGetSubMatrix(H_temp,isj3,isi1,1,MAT_INITIAL_MATRIX,&mat_j3i1);
    MatGetSubMatrix(H_temp,isj5,isi1,1,MAT_INITIAL_MATRIX,&mat_j5i1);
    MatGetSubMatrix(H_temp,isj7,isi1,1,MAT_INITIAL_MATRIX,&mat_j7i1);
    MatGetSubMatrix(H_temp,isj4,isi2,1,MAT_INITIAL_MATRIX,&mat_j4i2);
    MatGetSubMatrix(H_temp,isj6,isi2,1,MAT_INITIAL_MATRIX,&mat_j6i2);
    MatGetSubMatrix(H_temp,isj4,isi3,1,MAT_INITIAL_MATRIX,&mat_j4i3);
    MatGetSubMatrix(H_temp,isj5,isi4,1,MAT_INITIAL_MATRIX,&mat_j5i4);
    MatGetSubMatrix(H_temp,isj6,isi5,1,MAT_INITIAL_MATRIX,&mat_j6i5);
    MatGetSubMatrix(H_temp,isj7,isi6,1,MAT_INITIAL_MATRIX,&mat_j7i6);
...
/*  
    MatSubMatrixUpdate(H_temp,mat_i1j2,isi1,isj2);  // up triangle
    MatSubMatrixUpdate(H_temp,mat_i1j3,isi1,isj3);
    MatSubMatrixUpdate(H_temp,mat_i1j5,isi1,isj5);
    MatSubMatrixUpdate(H_temp,mat_i1j7,isi1,isj7);
    MatSubMatrixUpdate(H_temp,mat_i2j4,isi2,isj4);
    MatSubMatrixUpdate(H_temp,mat_i2j6,isi2,isj6);
    MatSubMatrixUpdate(H_temp,mat_i3j4,isi3,isj4);
    MatSubMatrixUpdate(H_temp,mat_i4j5,isi4,isj5);
    MatSubMatrixUpdate(H_temp,mat_i5j6,isi5,isj6);
    MatSubMatrixUpdate(H_temp,mat_i6j7,isi6,isj7);
    MatSubMatrixUpdate(H_temp,mat_j2i1,isj2,isi1);  // low triangle
    MatSubMatrixUpdate(H_temp,mat_j3i1,isj3,isi1);
    MatSubMatrixUpdate(H_temp,mat_j5i1,isj5,isi1);
    MatSubMatrixUpdate(H_temp,mat_j7i1,isj7,isi1);
    MatSubMatrixUpdate(H_temp,mat_j4i2,isj4,isi2);
    MatSubMatrixUpdate(H_temp,mat_j6i2,isj6,isi2);
    MatSubMatrixUpdate(H_temp,mat_j4i3,isj4,isi3);
    MatSubMatrixUpdate(H_temp,mat_j5i4,isj5,isi4);
    MatSubMatrixUpdate(H_temp,mat_j6i5,isj6,isi5);
    MatSubMatrixUpdate(H_temp,mat_j7i6,isj7,isi6);
*/
...
// propagation by Cranck-Nicholson of APT and AIS1, implemented by PETSc. Define cn_factor=-i*dt/2.
  void Propagate_APT_AIS1 ( PetscScalar cn_factor, PetscScalar EXt, PetscScalar ELt, Mat H_static, IS isi1, IS isi2, IS isi3, IS isi4, IS isi5, IS isi6, IS isj2, IS isj3, IS isj4, IS isj5, IS isj6, IS isj7, KSP &ksp, PC &pc, Vec &psi )
  {
    ...
    //cout<< "*** Mat duplicate ***"<<endl;
    Mat H_temp;
    MatDuplicate(H_static, MAT_COPY_VALUES, &H_temp);  // H_temp = H_static, copy values and nonzero sturcture
    // Assembly_matrix_flush(H_temp);   // flushly assemble between setting values
    Assembly_matrix_final(H_temp);   // finally assemble after setting values brgore use
 
    //cout<< "*** Modify DME by laser fields ***"<<endl;
    DME_time_AIS1(EXt, ELt, isi1, isi2, isi3, isi3, isi5, isi6, isj2, isj3, isj4, isj5, isj6, isj7, H_temp);
    Assembly_matrix_final(H_temp);   // finally assemble after the last setting values
      // MatView(H_temp, PETSC_VIEWER_STDOUT_WORLD);
