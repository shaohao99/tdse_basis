...
// }  // end of parallel
 ...
 void Set_DME_GRASP (int n_basis, int n_dip, vector<int> idx_high, vector<int> idx_low, vector<double> dipole, Mat &H_dipole)
 {
   ...
   PetscScalar diag=complexd(0.,0.);
   for(int i=0; i<n_basis; i++){
     MatSetValue(H_dipole, i, i, diag, INSERT_VALUES);  // diagnal
   }  // end of i

   for(int i=1; i<=n_dip; i++){   // start from 1
     PetscInt idx1=idx_low[i]-1;  // shift 1 to 0
     PetscInt idx2=idx_high[i]-1;
     PetscScalar offdiag=complexd(dipole[i],0.);   // set matrix elements, complex value
     MatSetValue(H_dipole, idx1, idx2, offdiag, INSERT_VALUES);
     MatSetValue(H_dipole, idx2, idx1, offdiag, INSERT_VALUES);  // symmetric
   }  // end of i
...
// PETSc functions:
  ...
  void Create_Mat ( Mat &matrix, vector<int> nlocal, PetscInt n, PetscInt m )
  {
      ...
      MatSetType(matrix, MATMPIDENSE);  // MATMPIAIJ: sparse, MATMPIBAIJ: block spares, MATMPIDENSE: dense, 
...
// propagation by Cranck-Nicholson, implemented by PETSc. Define cn_factor=-i*dt/2.
  void PropagateCN_linear_PETSc ( PetscScalar cn_factor, PetscScalar Et, Vec diag, Mat H_dipole, KSP &ksp, PC &pc, Vec &psi, PetscViewer &viewer_test, PetscMPIInt rank )
  {
      ...
      if(rank==0)  cout<<"--- Et="<<real(Et)<<",   f_cn="<<cn_factor<<endl;
    ...
    MatScale(H_temp, cn_factor); // H_temp = -i*dt/2*H, make sure Mat is complex
      //MatView(H_temp, viewer_test);
    ...
    KSPSetTolerances(ksp, PETSC_DEFAULT, PETSC_DEFAULT, PETSC_DEFAULT, PETSC_DEFAULT); // default: 1.e-7; diverge: 1e-4
    ...
    KSPSolve(ksp, vec_temp, psi);   // solve H_left * psi(t+dt) = b, and save the solution to the new psi
     // KSPView(ksp,PETSC_VIEWER_STDOUT_WORLD); 
      ...
      //VecView(vec_temp, PETSC_VIEWER_STDOUT_WORLD);
      //VecView(psi, viewer_test);
      //VecView(vec_temp, viewer_test);
      //MatView(H_temp, viewer_test);
...
// DME offdiagonal nonzero structure, before time iteration
  void DME_index_AIS1(int nE, int n_isolate, PetscInt *i1, PetscInt *i2, PetscInt *j2, PetscInt *j3, PetscInt *j4, PetscInt *j5, PetscInt *j6, PetscInt *j7, PetscInt *i3, PetscInt *i4, PetscInt *i5, PetscInt *i6){  // globel index
...
// DME offdiagonal nonzero structure, before time iteration
  void DME_array_index_AIS1(int nE, int n_isolate, PetscInt i1[], PetscInt i2[], PetscInt j2[], PetscInt j3[], PetscInt j4[], PetscInt j5[], PetscInt j6[], PetscInt j7[], PetscInt i3[], PetscInt i4[], PetscInt i5[], PetscInt i6[]){  // globel index
...
// set DME values and save in pointer
  void DME_values_AIS1(int nE, vector<complexd> &ai1j2, vector<complexd> &ai1j3, vector<complexd> &ai1j5, vector<complexd> &ai1j7, vector<complexd> &ai2j4,  vector<complexd> &ai2j6,  vector<complexd> &ai2j5, vector<complexd> &ai3j4, vector<complexd> &ai4j5, vector<complexd> &ai5j6, vector<complexd> &ai6j7){

    //vector<complexd> vj2i1(1), vj3i1(nE), vj4i3(nE*nE);

    double factor=1.;  //1000.;
    ai1j2[0]=2.*factor;
    //vj2i1[0]=conj(ai1j2[0]);
     for(int i=0; i<nE; i++){
       ai1j3[i]=1.*factor;  // g,cotinuum, by XUV
       //vj3i1[i]=conj(ai1j3[i]);
       ai1j5[i]=1.*factor;
       ai1j7[i]=1.*factor;
       ai2j4[i]=4.*factor;  // a,cotinuum, by IR
       ai2j6[i]=4.*factor;
       ai2j5[i]=3.*factor;  // CI

       for(int j=0; j<nE; j++){
         int index=i*nE+j;
         ai3j4[index]=5.*factor;  // continuum,cotinuum, by IR
         //vj4i3[index]=conj(ai3j4[index]);
         ai4j5[index]=5.*factor;
         ai5j6[index]=5.*factor;
         ai6j7[index]=5.*factor;
       } 
     }
...
// set DME values and save in array
  void DME_array_values_AIS1(int nE, PetscScalar vi1j2[], PetscScalar vi1j3[], PetscScalar vi1j5[], PetscScalar vi1j7[], PetscScalar vi2j4[], PetscScalar vi2j6[], PetscScalar vi2j5[], PetscScalar vi3j4[], PetscScalar vi4j5[], PetscScalar vi5j6[], PetscScalar vi6j7[] )
{
     ...
     vi1j2[0]=2.*factor;
     for(int i=0; i<nE; i++){
       vi1j3[i]=1.*factor;  // g,cotinuum, by XUV
       vi1j5[i]=1.*factor;
       vi1j7[i]=1.*factor;
       vi2j4[i]=4.*factor;  // a,cotinuum, by IR
       vi2j6[i]=4.*factor;
       vi2j5[i]=3.*factor;  // CI

       for(int j=0; j<nE; j++){
         int index=i*nE+j;
         vi3j4[index]=5.*factor;  // continuum,cotinuum, by IR
         vi4j5[index]=5.*factor;         
         vi5j6[index]=5.*factor;         
         vi6j7[index]=5.*factor;   
       }
     }
...
// set init DME 
 void DME_pointer_init_AIS1(int nE, vector<double> E_eigen_array, PetscInt *i1, PetscInt *i2, PetscInt *j2, PetscInt *j3, PetscInt *j4, PetscInt *j5, PetscInt *j6, PetscInt *j7, PetscInt *i3, PetscInt *i4, PetscInt *i5, PetscInt *i6, vector<complexd> ai1j2, vector<complexd> ai1j3, vector<complexd> ai1j5, vector<complexd> ai1j7, vector<complexd> ai2j4,  vector<complexd> ai2j6,  vector<complexd> ai2j5, vector<complexd> ai3j4, vector<complexd> ai4j5, vector<complexd> ai5j6, vector<complexd> ai6j7, Mat &H_temp){
    ...
    vi1j2[0]=ai1j2[0];  // g,a, by XUV
    vj2i1[0]=conj(vi1j2[0]);
    for(int i=0; i<nE; i++){
       vi1j3[i]=ai1j3[i];  // g,cotinuum, by XUV
       vj3i1[i]=conj(vi1j3[i]);

       vi1j5[i]=ai1j5[i];
       vj5i1[i]=conj(vi1j5[i]);

       vi1j7[i]=ai1j7[i];
       vj7i1[i]=conj(vi1j7[i]);

       vi2j4[i]=ai2j4[i];  // a,cotinuum, by IR
       vj4i2[i]=conj(vi2j4[i]);

       vi2j6[i]=ai2j6[i];
       vj6i2[i]=conj(vi2j6[i]);
       ...
       for(int j=0; j<nE; j++){
         int index=i*nE+j;
         vi3j4[index]=ai3j4[index];  // continuum,cotinuum, by IR
         vj4i3[index]=conj(vi3j4[index]);

         vi4j5[index]=ai4j5[index]; 
         vj5i4[index]=conj(vi4j5[index]);

         vi5j6[index]=ai5j6[index]; 
         vj6i5[index]=conj(vi5j6[index]);

         vi6j7[index]=ai6j7[index]; 
         vj7i6[index]=conj(vi6j7[index]);
...
// set time DME 
 void DME_array_time_AIS1(PetscScalar EXt, PetscScalar ELt, int nE, PetscInt i1[], PetscInt i2[], PetscInt j2[], PetscInt j3[], PetscInt j4[], PetscInt j5[], PetscInt j6[], PetscInt j7[], PetscInt i3[], PetscInt i4[], PetscInt i5[], PetscInt i6[], PetscScalar vi1j2[], PetscScalar vi1j3[], PetscScalar vi1j5[], PetscScalar vi1j7[], PetscScalar vi2j4[], PetscScalar vi2j6[], PetscScalar vi3j4[], PetscScalar vi4j5[], PetscScalar vi5j6[], PetscScalar vi6j7[], Mat &H_temp){
    ...
    PetscScalar val1[1],val2[nE],val3[nE*nE];

    val1[0]=vi1j2[0]*EXt;  // g,a, by XUV
    MatSetValues(H_temp,1,i1,1,j2,val1,INSERT_VALUES);

    for(int i=0; i<nE; i++) val2[i]=vi1j3[i]*EXt;  // g,cotinuum, by XUV
    MatSetValues(H_temp,1,i1,nE,j3,val2,INSERT_VALUES);
    for(int i=0; i<nE; i++) val2[i]=vi1j5[i]*EXt;
    MatSetValues(H_temp,1,i1,nE,j5,val2,INSERT_VALUES);
    for(int i=0; i<nE; i++) val2[i]=vi1j7[i]*EXt;
    MatSetValues(H_temp,1,i1,nE,j7,val2,INSERT_VALUES);
    for(int i=0; i<nE; i++) val2[i]=vi2j4[i]*ELt;  // a,cotinuum, by IR
    MatSetValues(H_temp,1,i2,nE,j4,val2,INSERT_VALUES);
    for(int i=0; i<nE; i++) val2[i]=vi2j6[i]*ELt;
    MatSetValues(H_temp,1,i2,nE,j6,val2,INSERT_VALUES);

    for(int i=0; i<nE; i++)  for(int j=0; j<nE; j++){
         int index=i*nE+j;
         val3[index]=vi3j4[index]*ELt;  // continuum,cotinuum, by IR
    }
    //MatSetValues(H_temp,nE,i3,nE,j4,val3,INSERT_VALUES);
    MatSetValuesBlocked(H_temp,nE,i3,nE,j4,val3,INSERT_VALUES);

/*
    for(int i=0; i<nE; i++)  for(int j=0; j<nE; j++){
         int index=i*nE+j;
         val3[index]=vi4j5[index]*ELt;
    }
    MatSetValues(H_temp,nE,i4,nE,j5,val3,INSERT_VALUES);

    for(int i=0; i<nE; i++)  for(int j=0; j<nE; j++){
         int index=i*nE+j;
         val3[index]=vi5j6[index]*ELt;
    }
    MatSetValues(H_temp,nE,i5,nE,j6,val3,INSERT_VALUES);

    for(int i=0; i<nE; i++)  for(int j=0; j<nE; j++){
         int index=i*nE+j;
         val3[index]=vi6j7[index]*ELt;
    }
    MatSetValues(H_temp,nE,i6,nE,j7,val3,INSERT_VALUES);
*/

    /*MatSetOption(H_temp,MAT_HERMITIAN,PETSC_TRUE);  // before assembly begin, set Hermitian
    MatAssemblyBegin(H_temp, MAT_FINAL_ASSEMBLY);
    MatAssemblyEnd(H_temp, MAT_FINAL_ASSEMBLY);*/

 }  // end DME array set time mat
...
// set time DME 
 void DME_value_time_AIS1(PetscScalar EXt, PetscScalar ELt, int nE, PetscInt i1[], PetscInt i2[], PetscInt j2[], PetscInt j3[], PetscInt j4[], PetscInt j5[], PetscInt j6[], PetscInt j7[], PetscInt i3[], PetscInt i4[], PetscInt i5[], PetscInt i6[], PetscScalar vi1j2[], PetscScalar vi1j3[], PetscScalar vi1j5[], PetscScalar vi1j7[], PetscScalar vi2j4[], PetscScalar vi2j6[], PetscScalar vi3j4[], PetscScalar vi4j5[], PetscScalar vi5j6[], PetscScalar vi6j7[], Mat &H_temp){
    ...
    value=vi1j2[0]*EXt;  // g,a, by XUV
    MatSetValue(H_temp,i1[0],j2[0],value,INSERT_VALUES);

    for(int i=0; i<nE; i++){
       value=vi1j3[i]*EXt;  // g,cotinuum, by XUV
       MatSetValue(H_temp,i1[0],j3[i],value,INSERT_VALUES);
       value=vi1j5[i]*EXt;
       MatSetValue(H_temp,i1[0],j5[i],value,INSERT_VALUES);
       value=vi1j7[i]*EXt;
       MatSetValue(H_temp,i1[0],j7[i],value,INSERT_VALUES);
       value=vi2j4[i]*ELt;  // a,cotinuum, by IR
       MatSetValue(H_temp,i2[0],j4[i],value,INSERT_VALUES);
       value=vi2j6[i]*ELt;
       MatSetValue(H_temp,i2[0],j6[i],value,INSERT_VALUES);

       for(int j=0; j<nE; j++){
         int index=i*nE+j;
         value=vi3j4[index]*ELt;  // continuum,cotinuum, by IR
         MatSetValue(H_temp,i3[i],j4[j],value,INSERT_VALUES);
         value=vi4j5[index]*ELt;
         MatSetValue(H_temp,i4[i],j5[j],value,INSERT_VALUES);
         value=vi5j6[index]*ELt;
         MatSetValue(H_temp,i5[i],j6[j],value,INSERT_VALUES);
         value=vi6j7[index]*ELt;
         MatSetValue(H_temp,i6[i],j7[j],value,INSERT_VALUES);
       }
    }
...
// set time DME 
 void DME_pointer_time_AIS1(PetscScalar EXt, PetscScalar ELt, int nE, PetscInt *i1, PetscInt *i2, PetscInt *j2, PetscInt *j3, PetscInt *j4, PetscInt *j5, PetscInt *j6, PetscInt *j7, PetscInt *i3, PetscInt *i4, PetscInt *i5, PetscInt *i6, vector<complexd> ai1j2, vector<complexd> ai1j3, vector<complexd> ai1j5, vector<complexd> ai1j7, vector<complexd> ai2j4,  vector<complexd> ai2j6, vector<complexd> ai3j4, vector<complexd> ai4j5, vector<complexd> ai5j6, vector<complexd> ai6j7, Mat &H_temp){
    ...
    vi1j2[0]=ai1j2[0]*EXt;  // g,a, by XUV
    vj2i1[0]=conj(vi1j2[0]);
    for(int i=0; i<nE; i++){
       vi1j3[i]=ai1j3[i]*EXt;  // g,cotinuum, by XUV
       vj3i1[i]=conj(vi1j3[i]);
       ...
       vi2j4[i]=ai2j4[i]*ELt;  // a,cotinuum, by IR
       vj4i2[i]=conj(vi2j4[i]);
       ...
       for(int j=0; j<nE; j++){
         int index=i*nE+j;
         vi3j4[index]=ai3j4[index]*ELt;  // continuum,cotinuum, by IR
         vj4i3[index]=conj(vi3j4[index]);

         vi4j5[index]=ai4j5[index]*ELt; 
         vj5i4[index]=conj(vi4j5[index]);

         vi5j6[index]=ai5j6[index]*ELt; 
         vj6i5[index]=conj(vi5j6[index]);

         vi6j7[index]=ai6j7[index]*ELt; 
         vj7i6[index]=conj(vi6j7[index]);

       } 
    }

    //cout<<"^^^ Mat set values ^^^"<<endl;
// offdiagonal: up triangle
    MatSetValues(H_temp,1,i1,1,j2,vi1j2,INSERT_VALUES);
    MatSetValues(H_temp,1,i1,nE,j3,vi1j3,INSERT_VALUES);
    MatSetValues(H_temp,1,i1,nE,j5,vi1j5,INSERT_VALUES);
...
// offdiagonal: up triangle
    ...
    MatSetValues(H_temp,1,i2,nE,j4,vi2j4,INSERT_VALUES);
    MatSetValues(H_temp,1,i2,nE,j6,vi2j6,INSERT_VALUES);
    MatSetValues(H_temp,1,i2,nE,j6,vi2j5,INSERT_VALUES);
    MatSetValues(H_temp,nE,i3,nE,j4,vi3j4,INSERT_VALUES);
    MatSetValues(H_temp,nE,i4,nE,j5,vi4j5,INSERT_VALUES);
    MatSetValues(H_temp,nE,i5,nE,j6,vi5j6,INSERT_VALUES);
    MatSetValues(H_temp,nE,i6,nE,j7,vi6j7,INSERT_VALUES);
// offdiagonal: down triangle, Hermitian
    ...
    MatSetValues(H_temp,nE,j4,1,i2,vj4i2,INSERT_VALUES);
    MatSetValues(H_temp,nE,j6,1,i2,vj6i2,INSERT_VALUES);
    MatSetValues(H_temp,nE,j6,1,i2,vj5i2,INSERT_VALUES);
    MatSetValues(H_temp,nE,j4,nE,i3,vj4i3,INSERT_VALUES);
    MatSetValues(H_temp,nE,j5,nE,i4,vj5i4,INSERT_VALUES);
    MatSetValues(H_temp,nE,j6,nE,i5,vj6i5,INSERT_VALUES);
    MatSetValues(H_temp,nE,j7,nE,i6,vj7i6,INSERT_VALUES);
...
// Get values from mat to array
    ...
    MatDenseGetArray(mat_i1j2,&vi1j2);  // up triangle
    MatDenseGetArray(mat_i1j3,&vi1j3);
    MatDenseGetArray(mat_i1j5,&vi1j5);
    MatDenseGetArray(mat_i1j7,&vi1j7);
    MatDenseGetArray(mat_i2j4,&vi2j4);
    MatDenseGetArray(mat_i2j6,&vi2j6);
    MatDenseGetArray(mat_i3j4,&vi3j4);
    MatDenseGetArray(mat_i4j5,&vi4j5);
    MatDenseGetArray(mat_i5j6,&vi5j6);
    MatDenseGetArray(mat_i6j7,&vi6j7);
    MatDenseGetArray(mat_j2i1,&vj2i1);  // down triangle
    MatDenseGetArray(mat_j3i1,&vj3i1); 
    MatDenseGetArray(mat_j5i1,&vj5i1); 
    MatDenseGetArray(mat_j7i1,&vj7i1); 
    MatDenseGetArray(mat_j4i2,&vj4i2); 
    MatDenseGetArray(mat_j6i2,&vj6i2); 
    MatDenseGetArray(mat_j4i3,&vj4i3); 
    MatDenseGetArray(mat_j5i4,&vj5i4); 
    MatDenseGetArray(mat_j6i5,&vj6i5); 
    MatDenseGetArray(mat_j7i6,&vj7i6); 
...
// sub matrix restore
    MatDenseRestoreArray(mat_i1j2,&vi1j2);  // up triangle
    MatDenseRestoreArray(mat_i1j3,&vi1j3);
    MatDenseRestoreArray(mat_i1j5,&vi1j5);
    MatDenseRestoreArray(mat_i1j7,&vi1j7);
    MatDenseRestoreArray(mat_i2j4,&vi2j4);
    MatDenseRestoreArray(mat_i2j6,&vi2j6);
    MatDenseRestoreArray(mat_i3j4,&vi3j4);
    MatDenseRestoreArray(mat_i4j5,&vi4j5);
    MatDenseRestoreArray(mat_i5j6,&vi5j6);
    MatDenseRestoreArray(mat_i6j7,&vi6j7);
    MatDenseRestoreArray(mat_j2i1,&vj2i1);  // down triangle
    MatDenseRestoreArray(mat_j3i1,&vj3i1);
    MatDenseRestoreArray(mat_j5i1,&vj5i1);
    MatDenseRestoreArray(mat_j7i1,&vj7i1);
    MatDenseRestoreArray(mat_j4i2,&vj4i2);
    MatDenseRestoreArray(mat_j6i2,&vj6i2);
    MatDenseRestoreArray(mat_j4i3,&vj4i3);
    MatDenseRestoreArray(mat_j5i4,&vj5i4);
    MatDenseRestoreArray(mat_j6i5,&vj6i5);
    MatDenseRestoreArray(mat_j7i6,&vj7i6);
