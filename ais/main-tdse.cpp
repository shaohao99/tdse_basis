...
// to set nonzero structure of a matrix (using INSERT_VALUES) consumes time, it is important to do it before time evolution.
  ...
  PetscInt i1[1], i2[1], j2[1], j3[nE], j4[nE], j5[nE], j6[nE], j7[nE], i3[nE], i4[nE], i5[nE], i6[nE];
  DME_array_index_AIS1(nE, n_isolate, i1, i2, j2, j3, j4, j5, j6, j7, i3, i4, i5, i6);  // define nonzero index

  ofstream  output_str;
  Open_outFile( output_str, output_folder + "/index.dat");
  if(rank==0){
    output_str<<"i1: "<<i1[0]<<endl;
    output_str<<"i2: "<<i2[0]<<endl;
    output_str<<"j2: "<<j2[0]<<endl;
    for(int i=0; i<nE; i++){
      output_str<<"i3: "<<i3[i]<<endl;
      output_str<<"i4: "<<i4[i]<<endl;
      output_str<<"i5: "<<i5[i]<<endl;
      output_str<<"i6: "<<i6[i]<<endl;
      output_str<<"j3: "<<j3[i]<<endl;
      output_str<<"j4: "<<j4[i]<<endl;
      output_str<<"j5: "<<j5[i]<<endl;
      output_str<<"j6: "<<j6[i]<<endl;
      output_str<<"j7: "<<j7[i]<<endl;
    }
  }


// calculate static dipolemoment
  if(rank==0) cout<<"--- calculate and store DME in arrays ---"<<endl;
  PetscScalar vi1j2[1], vi1j3[nE], vi1j5[nE], vi1j7[nE], vi2j4[nE], vi2j6[nE], vi2j5[nE], vi3j4[nE*nE], vi4j5[nE*nE], vi5j6[nE*nE], vi6j7[nE*nE];
  DME_array_values_AIS1(nE, vi1j2, vi1j3, vi1j5, vi1j7, vi2j4, vi2j6, vi2j5, vi3j4, vi4j5, vi5j6, vi6j7);
  
  ofstream  output_dip;
  Open_outFile( output_dip, output_folder + "/dipole.dat");
  if(rank==0){
    output_dip<<"vi1j2: "<<vi1j2[0]<<endl;
    for(int i=0; i<nE; i++){
      output_dip<<"vi1j3: "<<vi1j3[i]<<endl;
      output_dip<<"vi2j4: "<<vi2j4[i]<<endl;
      output_dip<<"vi2j5: "<<vi2j5[i]<<endl;
      for(int j=0; j<nE; j++){
        int index=i*nE+j;
        output_dip<<"vi3j4: "<<vi3j4[index]<<endl;
      }
    }
  }
...
// ----------- set dipole moments, energies and nonzero structures of Ham
  ...
  DME_array_init_AIS1(nE, E_eigen_array, i1, i2, j2, j3, j4, j5, j6, j7, i3, i4, i5, i6, vi1j2, vi1j3, vi1j5, vi1j7, vi2j4, vi2j6,vi2j5, vi3j4, vi4j5, vi5j6, vi6j7, Ham);
...
// =========== time propagation ===============
  ...
  for(int i=0; i<nt; i++){  
    ...
    DME_array_time_AIS1(EXt, ELt, nE, i1, i2, j2, j3, j4, j5, j6, j7, i3, i4, i5, i6, vi1j2, vi1j3, vi1j5, vi1j7, vi2j4, vi2j6, vi3j4, vi4j5, vi5j6, vi6j7, Ham);  // update values of part of Ham in time, not changing the nz structure
//    DME_value_time_AIS1(EXt, ELt, nE, i1, i2, j2, j3, j4, j5, j6, j7, i3, i4, i5, i6, vi1j2, vi1j3, vi1j5, vi1j7, vi2j4, vi2j6, vi3j4, vi4j5, vi5j6, vi6j7, Ham);  // update values of part of Ham in time, not changing the nz structure
