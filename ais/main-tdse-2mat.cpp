...
// ----------- set dipole moments, energies and nonzero structures of Ham
  ...
  if(rank==0) cout<<"--- set initial values of Ham by arrays ---"<<endl;
...
//  DME_array_static_AIS1(nE, E_eigen_array, i2, j5, ai2j5, Ham);  // set only static parts
...
// =========== time propagation ===============
  ...
  for(int i=0; i<nt; i++){  
    ...
    Mat H_temp;
    MatDuplicate(Ham, MAT_COPY_VALUES, &H_temp);  // H_temp = H_static, copy values and nonzero structure
    Assembly_matrix_flush(H_temp);
    //Assembly_matrix_final(H_temp);
    //MatSetUp(H_temp); // setup before using MatSetValues
    DME_array_time_AIS1(EXt, ELt, nE, i1, i2, j2, j3, j4, j5, j6, j7, i3, i4, i5, i6, ai1j2, ai1j3, ai1j5, ai1j7, ai2j4, ai2j6, ai3j4, ai4j5, ai5j6, ai6j7, H_temp);  // update values and nonzero structure of H_temp in time
    //DME_full_time_AIS1(EXt, ELt, nE, E_eigen_array, i1, i2, j2, j3, j4, j5, j6, j7, i3, i4, i5, i6, ai1j2, ai1j3, ai1j5, ai1j7, ai2j4, ai2j6, ai2j5, ai3j4, ai4j5, ai5j6, ai6j7, H_temp);  // update values and nonzero structure of H_temp in time
    Assembly_matrix_final(H_temp);  // assemble after set values before use

    /*if(i%2000==0){
      MatView(H_temp, viewmattemp);
    }*/

    //if(rank==0) cout<<"### propagate ###"<<endl;
    Propagate_APT_AIS1 ( cn_factor, H_temp, ksp, pc, psi );  // propagate 1 time step

    MatDestroy(&H_temp);  // destroy after use
